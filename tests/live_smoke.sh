...
chk "PWA brief served (standalone)" 'curl -s -m 20 "https://moethiago.github.io/Moes-app/brief/index.html" | grep -q "apple-mobile-web-app-capable"'
chk "brief route answers (read-only, no build)" 'curl -s -m 20 "$B/feed?brief=1" | grep -q "\"cached\""'
